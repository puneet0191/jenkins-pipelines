                        ...
                        ssh -i "${KEY_PATH}" -o ConnectTimeout=1 -o StrictHostKeyChecking=no ${USER}@\$(cat IP) "
                                ...
                                if [[ \$CLIENT_VERSION = dev-latest ]]; then
                                    ...
                                    if [[ \$PMM_VERSION == pmm1 ]]; then
                                        ...
                                        if [[ \$CLIENT_INSTANCE == yes ]]; then
                                            pmm-agent setup --config-file=$PWD/pmm2-client/config/pmm-agent.yaml --server-address=\$SERVER_IP:443 --server-insecure-tls --server-username=admin --server-password=admin --trace
                                        else
                            ...
                            if [[ \$PMM_VERSION == pmm2 ]]; then
                                if [[ \$CLIENT_VERSION == dev-latest ]]; then
                                    pmm-admin --version
                                    if [[ \$CLIENT_INSTANCE == yes ]]; then
                                        sudo pmm-agent setup --server-address=\$SERVER_IP:443 --server-insecure-tls --server-username=admin --server-password=admin --trace
                                    else
                                        sudo pmm-agent setup --server-address=\$IP:443 --server-insecure-tls --server-username=admin --server-password=admin --trace
                                    fi
                                    sleep 10
